...
#######################################
# Function to read CSV (now returns headers)
#######################################
def read_csv_to_dict(file_path):
    ...
    with p.open(mode='r', newline='', encoding='utf-8') as file:
        ...
        try:
            header = next(reader)[1:]  # read all time slot labels (skip first column)
...
#######################################
# Display Schedule Function (uses header time labels)
#######################################
def display_schedule(schedule, ratings, time_labels, title, co_r, mut_r):
    total_rating = 0
    results = []
    for time_slot, program in enumerate(schedule):
        label = time_labels[time_slot] if time_slot < len(time_labels) else f"Slot {time_slot+1}"
        rating = ratings[program][time_slot]
        total_rating += rating
        results.append({
            "Time Slot": f"{label}:00" if label.isdigit() else label,
            "Program": program,
            "Rating": rating
        })
...
st.write("This app finds the best TV program schedule using a Genetic Algorithm. Upload your CSV and experiment with parameters!")
...
if uploaded_file:
    ...
    ratings, headers = read_csv_to_dict(temp_path)
    ...
    if st.button("🚀 Run All Trials"):
        ...
        display_schedule(best_default, ratings, headers, "🟩 Default Run Results", default_CO_R, default_MUT_R)
        ...
        for i, (co_r, mut_r) in enumerate(trial_params, start=1):
            best_trial = genetic_algorithm(ratings, all_programs,
                                           crossover_rate=co_r,
                                           mutation_rate=mut_r)
            display_schedule(best_trial, ratings, headers, f"🧪 Trial {i} Results", co_r, mut_r)

else:
    st.info("Please set parameters above and upload your CSV file to continue.")
